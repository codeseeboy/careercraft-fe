        ...
        <CardContent className="px-2 py-2">
          <div className="flex justify-center border-b">
            <div className="grid grid-cols-2 w-full max-w-md">
              <div 
                className={`px-6 py-3 text-center font-medium cursor-pointer transition-colors border-b-2 ${activeTab === 'search' ? 'border-primary text-primary' : 'border-transparent hover:text-primary/70'}`}
                onClick={() => setActiveTab('search')}
              >
                Search Jobs
              </div>
              <div 
                className={`px-6 py-3 text-center font-medium cursor-pointer transition-colors border-b-2 ${activeTab === 'saved' ? 'border-primary text-primary' : 'border-transparent hover:text-primary/70'}`}
                onClick={() => setActiveTab('saved')}
              >
                Saved Jobs {Object.keys(savedJobs).length > 0 && <span className="ml-1 text-xs bg-secondary rounded-full px-1.5 py-0.5">{Object.keys(savedJobs).length}</span>}
              </div>
            </div>
          </div>
        </CardContent>
